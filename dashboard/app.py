...
try:
    ...
    # IBKR connection dropped mid-session (e.g. a brief network hiccup).
    # Clear the cached connection/feeds so the next auto-refresh reconnects
    # and rebuilds everything from scratch, instead of crashing outright.
    st.warning(f"Lost connection to IBKR ({e}) — reconnecting automatically...")
    for key in ("ib", "feed", "tape_feed", "options_feed", "internals_feed"):
